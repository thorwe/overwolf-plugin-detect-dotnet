// props Aaron Stebner(http ://blogs.msdn.com/b/astebner/archive/2009/06/16/9763379.aspx)
...
namespace utils {
	...
	******************************************************************/
	...
	int GetSPLevel(const std::wstring& dotNetVersion) {
		...
		else if (dotNetVersion == L"4.5") {
			return GetNetfxSPLevel(g_szNetfx45RegKeyName, g_szNetfx40SPxRegValueName);
		}
		else if (dotNetVersion == L"4.0") {
			return GetNetfxSPLevel(g_szNetfx40FullRegKeyName, g_szNetfx40SPxRegValueName);
		}
		else if (dotNetVersion == L"4.0C") {
			return GetNetfxSPLevel(g_szNetfx40ClientRegKeyName, g_szNetfx40SPxRegValueName);
		}
		else if (dotNetVersion == L"3.5") {
			return GetNetfxSPLevel(g_szNetfx35RegKeyName, g_szNetfxStandardSPxRegValueName);
		}
		else if (dotNetVersion == L"3.0") {
			return GetNetfxSPLevel(g_szNetfx30SpRegKeyName, g_szNetfxStandardSPxRegValueName);
		}
		else if (dotNetVersion == L"2.0") {
			return GetNetfxSPLevel(g_szNetfx20RegKeyName, g_szNetfxStandardSPxRegValueName);
		}
		else if (dotNetVersion == L"1.1") {
			return GetNetfxSPLevel(g_szNetfx11RegKeyName, g_szNetfxStandardSPxRegValueName);
		}
		else if (dotNetVersion == L"1.0") {
			return GetNetfx10SPLevel();
		}
		return -1;
	}
